...
public class LevelGeneration : MonoBehaviour
{
    ...
    int gridSizeX, gridSizeY, numberOfRooms = 21;
    private bool AddBeginRoom = true;
    private bool roomboss = true;
    private int yFirstRoom;
    private int xFirstRoom;
    ...
    void selectRoomBoss(GameObject roomboss, Vector2 position, Vector2 PositionRoom)
    {
        rendPrefab = roomBossU;
        var clone = Object.Instantiate(roomboss, position, Quaternion.identity);
        Component[] yolo = clone.GetComponentsInChildren(typeof(RoomTransition));
        for (var j = 0; j < yolo.Length; j++)
        {
            yolo[j].GetComponent<RoomTransition>().SetPosRoom(PositionRoom);
        }
    ...
    void SelectPrefab(Vector2 pos, bool u, bool d, bool r, bool l, int i, bool BeginRoom, Vector2 PosRoom)
    {
        ...
        if (d)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (roomboss)
                    {
                        selectRoomBoss(roomBossD, pos, PosRoom);
                        roomboss = false;
                    }
                    else
                    {
        ...
        if (r)
        {
            ...
            else
            {
                ...
                else
                {
                    if (roomboss)
                    {
                        selectRoomBoss(roomBossR, pos, PosRoom);
                        roomboss = false;
                    }
                    else
                    {
        ...
        else
        {
            ...
            else
            {
                if (roomboss)
                {
                    selectRoomBoss(roomBossL, pos, PosRoom);
                    roomboss = false;
                }
                else
                {
    ...
    void DrawMap()
    {
        ...
        foreach (Room room in rooms)
        {
            ...
            {
                SelectPrefab(drawPos, up, down, right, left, i, false, room.GetRoom());
            }
            i++;
        }
